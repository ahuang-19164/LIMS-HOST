...
using Yichen.Net.IRepository.UnitOfWork;
using Yichen.Comm.Model.Entities;
...
namespace Yichen.Net.Repository
{
    ...
    public class {{ModelClassName}}Repository : BaseRepository<{{ModelClassName}}>, I{{ModelClassName}}Repository
    {
       ...
       #region 实现重写增删改查操作==========================================================
        ...
        /// <returns></returns>
        public  async Task<WebApiCallBack> InsertAsync({{ModelClassName}} entity)
        {
        ...
        /// <returns></returns>
        public  async Task<WebApiCallBack> UpdateAsync({{ModelClassName}} entity)
        {
            var jm = new WebApiCallBack();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public  async Task<WebApiCallBack> UpdateAsync(List<{{ModelClassName}}> entity)
        {
            var jm = new WebApiCallBack();
        ...
        public  async Task<WebApiCallBack> HideByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Updateable<{{ModelClassName}}>().SetColumns(p => p.dstate == true).Where(p => p.id == Convert.ToInt32(id)).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            if (bl)
            {
       ...
       #region 获取缓存的所有数据==========================================================
        ...
        /// <returns></returns>
        public  async Task<IPageList<{{ModelClassName}}>> QueryPageAsync(Expression<Func<{{ModelClassName}}, bool>> predicate,
