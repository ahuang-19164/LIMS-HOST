...
namespace Yichen.Per.Repository
{
    ...
    ///  接口实现
    /// </summary>
    public class SampleInfoDeleteRepository : BaseRepository<SampleInfoDelete>, ISampleInfoDeleteRepository
    {
        private readonly IUnitOfWork _unitOfWork;
        public SampleInfoDeleteRepository(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
       ...
       #region 实现重写增删改查操作==========================================================
        ...
        /// <returns></returns>
        public  async Task<WebApiCallBack> InsertAsync(SampleInfoDelete entity)
        {
        ...
        /// <returns></returns>
        public  async Task<WebApiCallBack> UpdateAsync(SampleInfoDelete entity)
        {
            var jm = new WebApiCallBack();

            var oldModel = await DbClient.Queryable<SampleInfoDelete>().In(entity.id).SingleAsync();
            if (oldModel == null)
            {
            jm.msg = "不存在此信息";
            return jm;
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public  async Task<WebApiCallBack> UpdateAsync(List<SampleInfoDelete> entity)
        {
            var jm = new WebApiCallBack();
        ...
        public  async Task<WebApiCallBack> DeleteByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Deleteable<SampleInfoDelete>(id).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
        ...
        public  async Task<WebApiCallBack> DeleteByIdsAsync(int[] ids)
        {
            ...
            var bl = await DbClient.Deleteable<SampleInfoDelete>().In(ids).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            //if (bl)
            //{
        ...
        public  async Task<WebApiCallBack> HideByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Updateable<SampleInfoDelete>().SetColumns(p => p.dstate == true).Where(p => p.id == Convert.ToInt32(id)).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            //if (bl)
            //{
       ...
       #region 获取缓存的所有数据==========================================================
        ...
        ///// </summary>
        ///// <returns></returns>
        //public async Task<List<SampleInfoDelete>> GetCaChe()
        //{
        //    var cache = ManualDataCache.Instance.Get<List<SampleInfoDelete>>(GlobalConstVars.CacheSampleInfoDelete);
        //    if (cache != null)
        //    {
        //        return cache;
        //    }
        ...
        /////     更新cache
        ///// </summary>
        //public async Task<List<SampleInfoDelete>> UpdateCaChe()
        //{
        //    var list = await DbClient.Queryable<SampleInfoDelete>().With(SqlWith.NoLock).ToListAsync();
        //    ManualDataCache.Instance.Set(GlobalConstVars.CacheSampleInfoDelete, list);
        //    return list;
        //}
        ...
        /// <returns></returns>
        public  async Task<IPageList<SampleInfoDelete>> QueryPageAsync(Expression<Func<SampleInfoDelete, bool>> predicate,
            Expression<Func<SampleInfoDelete, object>> orderByExpression, OrderByType orderByType, int pageIndex = 1,
            int pageSize = 20, bool blUseNoLock = false)
        {
            RefAsync<int> totalCount = 0;
            List<SampleInfoDelete> page;
            if (blUseNoLock)
            {
                page = await DbClient.Queryable<SampleInfoDelete>()
                .OrderByIF(orderByExpression != null, orderByExpression, orderByType)
                .WhereIF(predicate != null, predicate).Select(p => new SampleInfoDelete
                {
                      id = p.id,
            ...
            var list = new PageList<SampleInfoDelete>(page, pageIndex, pageSize, totalCount);
            return list;
        }
