...
namespace Yichen.Comm.Services
{
    ...
    public class ToolsServices : IToolsServices
    {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> IllegalSqlContainsAny(string oldString)
        {
            var cache = ManualDataCache.MemoryCache.Get<string>(ToolsVars.IllegalWordsCahceName);
            if (string.IsNullOrEmpty(cache))
            {
                IFileProvider fileProvider = this._hostEnvironment.ContentRootFileProvider;
                IFileInfo fileInfo = fileProvider.GetFileInfo("illegalWord/IllegalSql.txt");
                ...
                cache = fileContent;
                ManualDataCache.MemoryCache.Set(ToolsVars.IllegalWordsCahceName, cache);
            }
        ...
        public string? UpLoadBase64ForLocalStorage(FilesStorageOptions options, MemoryStream memStream, int filesStorageLocation = (int)GlobalEnumVars.FilesStorageLocation.Admin)
        {
            ...
            bp.Save(filePath, ImageFormat.Jpeg);//注意保存路径
