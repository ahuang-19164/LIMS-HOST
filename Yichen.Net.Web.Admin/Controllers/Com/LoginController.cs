...
namespace Yichen.Net.Web.Admin.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<AdminUiCallBack> GetJwtToken([FromBody] FMLogin model)
        {
            ...
            var user = await _sysUserServices.QueryByClauseAsync(p => p.userName == model.userNo && p.passWord == model.password);
            if (user != null)
            {
                if (user.state == 1)
                {
                ...
                var userRoles = await _sysUserServices.GetUserRoleNameStr(model.userNo, model.password);
                //如果是基于用户的授权策略，这里要添加用户;如果是基于角色的授权策略，这里要添加角色
                var claims = new List<Claim> {
                        new Claim("userNO",model.userNo),
                        new Claim("userName",user.userName),
                        new Claim("Role", user.roles.ToString()),
                        new Claim("WebRole", user.roles.ToString()),
