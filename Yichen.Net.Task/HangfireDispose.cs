...
namespace Yichen.Net.Tasks
{
    public class HangfireDispose
    {
        ...
        public static void HangfireService()
        {
            ...
            //分发报告生成任务到Redis中
            RecurringJob.AddOrUpdate<ReportDispatchJOP>(s => s.Execute(), "0 0/5 * * * ? ", TimeZoneInfo.Local); // 每5分钟取消一次订单

            ///更新标本架状态信息
            //RecurringJob.AddOrUpdate<StoresShelfHandleJOP>(s => s.Execute(), "0 0 0/1 * * ? ", TimeZoneInfo.Local); // 每小时自动更新标本架状态信息
            RecurringJob.AddOrUpdate<StoresShelfHandleJOP>(s => s.Execute(), "0 0/5 * * * ?  ", TimeZoneInfo.Local); // 每5分钟自动更新标本架状态信息

            ///每小时自动更新标本记录状态信息
            //RecurringJob.AddOrUpdate<StroesRecordHandleJOP>(s => s.Execute(), "0 0 0/1 * * ? ", TimeZoneInfo.Local); // 每小时自动更新标本记录状态信息
            RecurringJob.AddOrUpdate<StroesRecordHandleJOP>(s => s.Execute(), "0 0/5 * * * ?  ", TimeZoneInfo.Local); // 每5分钟自动更新标本记录状态信息


            ////自动刷新系统缓存
            //RecurringJob.AddOrUpdate<AutoRefreshSystemDataJob>(s => s.Execute(), "0 0/5 * * * ? ", TimeZoneInfo.Local); // 每5分钟取消一次订单
