...
namespace Yichen.Net.Web.Host.Controllers
{
    ...
    public partial class UserController : ControllerBase
    {
        ...
        /// <returns></returns>
        //[HttpPost][Authorize]
        [HttpPost, Route("Logins"),AllowAnonymous]
        public async Task<WebApiCallBack> Logins(LoginInfo loginInfo, bool verificationCode = true)
        {

            string s = JsonConvert.SerializeObject(loginInfo).ToLower();
            ...
            if (loginInfo==null||loginInfo.Password==null)
                return new WebApiCallBack() { code = 1, status = false, msg = SysConstVars.LoginError };

            // 密码至少要包含6个字符，同时包含至少一个大写字母、一个小写字母、一个数字和一个特殊字符
            string pattern = @"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^\da-zA-Z]).{6,}$";
            if (!Regex.IsMatch(loginInfo.Password, pattern))
                return new WebApiCallBack() { code = 1,status=false, msg = SysConstVars.LoginError };
            return await _userServices.Login(loginInfo, verificationCode);
        ...
        {
            return await _userServices.RefreshToken();
        ...
        {
            return await _userServices.GetUserInfo();
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        //[HttpPost,Route("ChangePassword2")][Authorize]
        //[Authorize]
        [HttpPost, Route("ChangePassword")]
        [Authorize]
        public async Task<WebApiCallBack> ChangePassword(EditPwdPost entity)
        {
            var jm = new WebApiCallBack();
            ...
            jm = await _userServices.ChangePassword(entity.userid, entity.newpwd, entity.pwd);
        ...
        /// </summary>
        /// <returns></returns>
        //[HttpGet, Route("getVierificationCode")][Authorize]
        [HttpGet, Route("GetVierificationCode")]
        [AllowAnonymous]
        public WebApiCallBack GetVierificationCode()
        {
            return _userServices.GetVierificationCode();
        }
