...
namespace Yichen.System.Repository
{
    ...
    ///  接口实现
    /// </summary>
    public class MicrobeTestRepository : BaseRepository<MicrobeTest>, IMicrobeTestRepository
    {
        private readonly IUnitOfWork _unitOfWork;
        public MicrobeTestRepository(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
       ...
       #region 实现重写增删改查操作==========================================================
        ...
        public  async Task<WebApiCallBack> UpdateAsync(MicrobeTest entity)
        {
            ...
            var oldModel = await DbClient.Queryable<MicrobeTest>().In(entity.id).SingleAsync();
            if (oldModel == null)
            {
            jm.msg = "不存在此信息";
            return jm;
        ...
        public  async Task<WebApiCallBack> DeleteByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Deleteable<MicrobeTest>(id).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
        ...
        public  async Task<WebApiCallBack> DeleteByIdsAsync(int[] ids)
        {
            ...
            var bl = await DbClient.Deleteable<MicrobeTest>().In(ids).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            if (bl)
            {
        ...
        public  async Task<WebApiCallBack> HideByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Updateable<MicrobeTest>().SetColumns(p => p.dstate == true).Where(p => p.id == Convert.ToInt32(id)).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            if (bl)
            {
       ...
       #region 获取缓存的所有数据==========================================================
        ...
        {
            var cache = ManualDataCache.Instance.Get<List<MicrobeTest>>(GlobalConstVars.CacheMicrobeTest);
            if (cache != null)
            {
                return cache;
            }
        ...
        {
            var list = await DbClient.Queryable<MicrobeTest>().With(SqlWith.NoLock).ToListAsync();
            ManualDataCache.Instance.Set(GlobalConstVars.CacheMicrobeTest, list);
            return list;
        }
            ...
            int pageSize = 20, bool blUseNoLock = false)
        {
            ...
            List<MicrobeTest> page;
            if (blUseNoLock)
            {
                page = await DbClient.Queryable<MicrobeTest>()
                .OrderByIF(orderByExpression != null, orderByExpression, orderByType)
                .WhereIF(predicate != null, predicate).Select(p => new MicrobeTest
                {
                      id = p.id,
                no = p.no,
                names = p.names,
            ...
            var list = new PageList<MicrobeTest>(page, pageIndex, pageSize, totalCount);
            return list;
        }
